...
import os
import warnings
from regex import E # type: ignore
...
from peft import AdapterController,SoftPromptController
from peft.peft_configuration import BitfitConfig,AdapterConfig,PTConfig
...
class T5LayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6, peft_config=None):
        """
        ...
        self.bitfit = True if isinstance(peft_config, BitfitConfig) else False
        if self.bitfit:
...
class T5DenseGatedGeluDense(nn.Module):
    def __init__(self, config, peft_config=None):
        super().__init__()
        self.bitfit = peft_config.bitfit if peft_config is not None else False
...
class T5LayerFF(nn.Module):
    def __init__(self, config, peft_config=None):
        ...
        self.train_task_adapters = config.train_task_adapters and peft_config.add_adapter_in_feed_forward
        if self.train_task_adapters:
            peft_config.reduction_factor = peft_config.task_reduction_factor
            self.adapter_controller = AdapterController(peft_config)
        self.layer_norm = T5LayerNorm(
            config.d_model, eps=config.layer_norm_epsilon, peft_config=peft_config)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False, peft_config=None):
        super().__init__()
        self.bitfit = True if isinstance(peft_config, BitfitConfig) else False
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, peft_config=None):
        super().__init__()
        self.SelfAttention = T5Attention(
            config, has_relative_attention_bias=has_relative_attention_bias, peft_config=peft_config)
        self.layer_norm = T5LayerNorm(
            config.d_model, eps=config.layer_norm_epsilon, peft_config=peft_config)
        self.train_task_adapters = config.train_task_adapters and peft_config.add_adapter_in_self_attention
        if self.train_task_adapters:
            peft_config.reduction_factor = peft_config.task_reduction_factor
            self.adapter_controller = AdapterController(peft_config)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, peft_config=None):
        ...
        self.EncDecAttention = T5Attention(
            config, has_relative_attention_bias=False, peft_config=peft_config)
        self.layer_norm = T5LayerNorm(
            config.d_model, eps=config.layer_norm_epsilon, peft_config=peft_config)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, peft_config=None):
        ...
        self.layer.append(T5LayerFF(config, peft_config=peft_config))
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, peft_config=None,):
        ...
        if self.append_prefix:
            self.soft_prompt_controller = SoftPromptController(config=peft_config, model_shared=embed_tokens)
            
        self.peft_config = peft_config
        self.block = nn.ModuleList(
            [T5Block(self.per_layer_config(config, i, self.peft_config if isinstance(self.config,AdapterConfig) else None, self.is_decoder),
                     has_relative_attention_bias=bool(i == 0),
                     peft_config=peft_config) for i in range(config.num_layers)]
        )
    ...
    def update_soft_prompt_controller(self, embed_tokens):
        if self.append_prefix:
            print("Update embed tokens")
            self.soft_prompt_controller = SoftPromptController(self.peft_config, embed_tokens)
        else :
            print("Update fail")
            
    def update_soft_prompt_final(self, path, task = "final"):
        if self.append_prefix:
            print("Update final soft prompt")
            self.soft_prompt_controller.update_soft_prompt_final_to_evaluate(path,task=task)
        else :
            print("Update fail")
    ####################################################

    def per_layer_config(self, config, layer_id, peft_config, is_decoder):
        """Sets the train_task_adapter in the config, based on the information given."""
        def is_valid_layer(layer_id, peft_config, is_decoder):
            valid_layer_ids = peft_config.task_adapter_layers_encoder\
                if not is_decoder else peft_config.task_adapter_layers_decoder
        ...
        valid_task_adapter_layer_id = is_valid_layer(
            layer_id, peft_config, is_decoder)
        add_task_adapter = True if not is_decoder else peft_config.task_adapter_in_decoder
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            ######################################
            if self.append_prefix :
                inputs_embeds = self.soft_prompt_controller(input_embedding=inputs_embeds, task=task)
                input_shape = inputs_embeds.size()[:-1]
            ######################################
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, peft_config=None):
        ...
        if self.prefix_tuning:
            self.prefix_dim = peft_config.prefix_dim
        self.encoder = T5Stack(encoder_config, self.shared, peft_config=peft_config,)
        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        if config.train_task_adapters:
            decoder_config.train_task_adapters = peft_config.task_adapter_in_decoder
            
        
        self.decoder = T5Stack(decoder_config, self.shared, peft_config=peft_config,)
        ##########################################################
        ...
        self.device_map = None
    ################################################
    def update_encoder_soft_prompt_controller(self):
        # load shared after then init soft_prompt
        self.encoder.update_soft_prompt_controller(self.get_input_embeddings())
        
    def update_encoder_soft_promp_final(self,path, task = "final"):
        self.encoder.update_soft_prompt_final(path, task = task)
    #########################################
    ...
    def forward(
        ...
        if self.prefix_tuning:
            ...
            attention_mask = encoder.soft_prompt_controller.extent_attention_mask(attention_mask)
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        if self.prefix_tuning:
            ...
            if attention_mask is not None:
                encoder_kwargs['attention_mask'] = encoder.soft_prompt_controller.extent_attention_mask(attention_mask=attention_mask)
        ###################################################
