...
def compute_rouge(translation_corpus, reference_corpus):
    count = 0
    rouge_scores = {
        "rouge_1/f_score": 0,
        "rouge_1/r_score": 0,
    ...
    for (references, translation) in zip(reference_corpus, translation_corpus):
        ...
        for r in new_references:
            r_t_score = rouge_score(predictions=[t.lower()],references=[r.lower()])
            for metric in item_rouge_scores:
                item_rouge_scores[metric] = max(item_rouge_scores[metric],r_t_score[metric])

        for metric in item_rouge_scores:
            rouge_scores[metric] = rouge_scores[metric] + item_rouge_scores[metric]
    for metric in rouge_scores:
        rouge_scores[metric] = rouge_scores[metric] / count
    return rouge_scores