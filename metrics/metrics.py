...
TASK_TO_METRICS = {"mrpc": ["accuracy", "f1"],
                   ...
                   "paws": ["accuracy"], 
                   "e2e": ["rouge1","rouge2","rougeL","bleu","meteor"],
                   "common_gen": ["rouge1","rouge2","rougeL","bleu","meteor"],
                   "web_nlg": ["rouge1","rouge2","rougeL","bleu","meteor"],
                   }

logger = getLogger(__name__)

def compute_nlg_metrics(predictions, targets) -> dict:
    rouge_results = compute_rouge(
        translation_corpus=predictions,
        reference_corpus=targets,
    )
    bleu_results = compute_bleu(
        translation_corpus=predictions, 
        reference_corpus=targets,
    )
    meteor_results = compute_meteor(
        translation_corpus=predictions, 
        reference_corpus=targets,
    )
    return {
        "rouge1": round(rouge_results['rouge_1/f_score'], 4),
        "rouge2": round(rouge_results['rouge_2/f_score'], 4),
        "rougeL": round(rouge_results['rouge_l/f_score'], 4),
...
def build_compute_metrics_fn(compute_metric, task_names, eval_metrics, post_processors):
    """Builds a dictionary from each task to the task metric."""

    def tasks_metrics(compute_metrics, metric, processor):
        return functools.partial(compute_metrics, eval_metrics=metric, post_processor=processor) # type: ignore

    return {task: tasks_metrics(compute_metric,metric,processor) for task,metric,processor in zip 
                            (task_names,eval_metrics,post_processors)}
